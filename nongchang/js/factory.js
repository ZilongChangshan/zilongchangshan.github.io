...
import { CROPS, PRODUCTS, RECIPES } from './data.js';
import { updateStorageUI } from './storage.js';

let _checkLevelUp = null;
export function initFactoryDeps(checkLevelUpFn) {
...
export function startCrafting(productId) {
...
export function updateFactoryUI() {
    if (!elements.factoryTab || !elements.factoryTab.classList.contains('active')) return;

    renderFactory();

    const queueContainer = elements.factoryQueue;
